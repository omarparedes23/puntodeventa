  ...
  return (
    <aside className="w-56 shrink-0 h-screen sticky top-0 border-r bg-white flex flex-col">
      ...
      <div className="px-4 py-4 border-b">
        <p className="text-xs font-semibold text-gray-900 truncate">{empresa?.nombre_comercial ?? empresa?.razon_social ?? 'MarketPos'}</p>
        <p className="text-xs text-gray-400 truncate">{empresa?.ruc}</p>
      </div>
      ...
      <div className="px-3 py-3 border-t space-y-0.5">
        ...
        <div className="px-3 py-2.5 mt-1">
          <p className="text-xs font-medium text-gray-700 truncate">{perfil?.nombre}</p>
          <p className="text-xs text-gray-400 capitalize">{perfil?.rol?.replace('_', ' ')}</p>
        </div>
      </div>
    </aside>
  )
