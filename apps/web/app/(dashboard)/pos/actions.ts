...
import { fechaHoyLima } from '@marketpos/core'
import { Decimal } from 'decimal.js'
import { ventaSchema, type VentaInput } from '@marketpos/core'
import { calcularTotalesVenta } from '@marketpos/core'
...
export async function getMasVendidos(
  ...
  const { data, error } = await (supabase as any).rpc('get_mas_vendidos', {
    p_empresa_id: perfil.empresa_id,
    p_limit: limit,
  })

  if (error) return { data: null, error: 'Error al obtener productos más vendidos' }
